...
| `fisbcache-settings.json` | `main/fisbcachesettings.go` | Not registered in Storage Lifecycle; tiny | Yes (temp+fsync+rename) | On settings change, boot, Config Backup/Restore | **Refuses** via the shared guard before `MkdirAll` (added when the Rolling FIS-B Weather Cache was reconciled with this audit); load-side degrades safely to the *disabled* defaults |
| `fisb-weather-cache/` (cached FIS-B products) | `main/fisbcacherun.go`, `main/fisbcachestorage.go` (`docs/fisb-weather-cache.md`) | Registered in Storage Lifecycle as a bounded cache namespace (byte and entry budgets, expiry) | Yes (temp+fsync+rename via `storagelifecycle`) | Only when the cache **and** its persistence are enabled (both off by default); off the capture path, on the write worker | **Refuses** via the shared guard at directory creation and before every persisted entry; a refusal is logged and raises the cache's recovery-error flag, never fatal |
