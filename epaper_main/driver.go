...
// lutFullRefresh1Gray/lutPartialRefresh1Gray are the exact waveform
// tables Waveshare's own reference driver (EPD_3in7.c's lut_1Gray_DU and
// lut_1Gray_A2) loads via cmdLUTRegister before every full and partial
// 1-bit black/white display update, respectively. This driver previously
// relied entirely on the controller's own OTP-stored default waveform
// (selected via DisplayUpdateControl2's mode byte alone) and never
// loaded either of these - a real hardware-validation finding: a fully
// wired, digitally error-free panel produced no visible output and no
// refresh flicker at all even after every other vendor-verified Init()
// command was added, which is consistent with the OTP default not being
// a valid/complete waveform for this specific panel/controller pairing.
// 105 bytes each, transcribed verbatim - not hand-simulated - from the
// vendor's own source.
var lutFullRefresh1Gray = []byte{
...
var lutPartialRefresh1Gray = []byte{
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
func (d *Driver) Update(ctx context.Context, bitmap []byte, full bool) error {
	...
	// Load the waveform table for this refresh type before activating -
	// the vendor's own reference driver always does this immediately
	// after writing RAM and before Master Activate. A real hardware-
	// validation finding: this driver previously never loaded either
	// table at all, relying entirely on the controller's OTP-stored
	// default waveform - on real hardware this produced a fully wired,
	// digitally error-free, zero-flicker blank panel even after every
	// other vendor-verified Init() command was added, consistent with
	// the OTP default not being valid/complete for this panel/controller
	// pairing.
	lut := lutPartialRefresh1Gray
	if full {
		lut = lutFullRefresh1Gray
	}
	if err := d.cmdData(cmdLUTRegister, lut...); err != nil {
		return fmt.Errorf("load LUT: %w", err)
	}
