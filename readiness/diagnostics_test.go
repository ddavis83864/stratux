...
func TestBuildDiagnosticBundle_TruncatesLogLines(t *testing.T) {
	...
	b := BuildDiagnosticBundle(time.Now(), "2.0-pre5", "abc123", HealthReport{}, nil, lines, nil, "", nil, nil, nil, nil)
...
func TestWriteDiagnosticBundle_WritesReadableJSON(t *testing.T) {
	dir := t.TempDir()
	b := BuildDiagnosticBundle(time.Date(2026, 6, 1, 12, 0, 0, 0, time.UTC), "2.0-pre5", "abc123", HealthReport{Overall: StateReady}, map[string]interface{}{"DEBUG": false}, nil, nil, "", nil, nil, nil, nil)
...
func TestWriteDiagnosticBundle_PrunesOldBundles(t *testing.T) {
	dir := t.TempDir()
	for i := 0; i < 5; i++ {
		b := BuildDiagnosticBundle(time.Date(2026, 6, 1, 12, i, 0, 0, time.UTC), "v", "c", HealthReport{}, nil, nil, nil, "", nil, nil, nil, nil)
...
func TestWriteDiagnosticBundle_FailureDoesNotPanic(t *testing.T) {
	...
	b := BuildDiagnosticBundle(time.Now(), "v", "c", HealthReport{}, nil, nil, nil, "", nil, nil, nil, nil)
